...
def presence_message(account_name='Guest'):
    # out_msg = 'Некорректное сообщение для проверки'
    out_msg = {
            ACTION: PRESENCE,
            TIME: time(),
            USER: {
...
def main():
    ...
    try:
        ...
        LOGGER.info(f'Не указан IP-адрес и/или номер порта. '
                    f'Применено значение по-умолчанию - {DEFAULT_IP_ADDRESS}:{DEFAULT_PORT}.')
        server_addr = DEFAULT_IP_ADDRESS
        server_port = DEFAULT_PORT
    except ValueError:
        LOGGER.error('Значение порта может быть только числом в диапазоне от 1024 до 65535.')
        sys.exit(1)
